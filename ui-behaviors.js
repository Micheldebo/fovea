document.addEventListener('DOMContentLoaded', function () {
      ...
      (context) => {
        ...
        const ctx = gsap.context(() => {
          document.querySelectorAll('[data-parallax="trigger"]').forEach((trigger) => {
            ...
            // ✅ px-based (y/x) instead of yPercent/xPercent — avoids forced reflow
            const prop = direction === 'horizontal' ? 'x' : 'y';

            const scrubAttr = trigger.getAttribute('data-parallax-scrub');
            // ✅ numeric scrub instead of `true` — avoids double-smoothing with Lenis
            const scrub = scrubAttr ? parseFloat(scrubAttr) : 1;
            ...
            gsap.fromTo(
              ...
              { [prop]: startVal },
              {
                ...
                scrollTrigger: {
                  ...
                  scrub,
                  invalidateOnRefresh: true // ✅ safe resize recalc
                }
              }
            );
          });
  ...
  document.querySelectorAll('[data-accordion-css-init]').forEach(function (acc) {
    ...
    acc.addEventListener('click', function (e) {
      const tog = e.target.closest('[data-accordion-toggle]');
      if (!tog) return;
      const panel = tog.closest('[data-accordion-status]');
      if (!panel) return;
      const active = panel.getAttribute('data-accordion-status') === 'active';
